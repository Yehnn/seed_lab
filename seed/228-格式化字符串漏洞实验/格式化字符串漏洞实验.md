...
![此处输入图片的描述](https://doc.shiyanlou.com/document-uid8797labid734timestamp1526537796709.png)
...
![此处输入图片的描述](https://doc.shiyanlou.com/document-uid8797labid734timestamp1526621505844.png/wm)

> 12 和后面 %016llX 的个数都是自己随意输入的。
>
> 12 的十六进制码就是 0x000C ，可以看到输出中 12 的十六进制在第 8 个位置上，这样我们就能确定格式化字符串的位置了。

2.输入 `secret[1]` 的地址，记得做进制转换，同时在格式字符串中加入%s。

![此处输入图片的描述](https://doc.shiyanlou.com/document-uid8797labid734timestamp1526622146797.png/wm)
...
1.只要求修改，不要求改成什么。简单！不明白 `%n`  用法的可以往前回顾一下。

![此处输入图片的描述](https://doc.shiyanlou.com/document-uid8797labid734timestamp1526622910954.png/wm)

大功告成 x2！可以看到已经变成 0x77 了。

###3.1.3 修改 secret[1]为期望值

1.要改成自己期望的值，比如改成十进制 1000，咋办？手动填充 1000 个岂不累死？！可以用格式化字符填充嘛！

![此处输入图片的描述](https://doc.shiyanlou.com/document-uid8797labid734timestamp1526628878497.png/wm)

> 从图上得知，`secret[1]` 的地址是 `0x1134014`，换算成十进制就是 `18038804`。
>
...
现在让我们把第一个 scanf 语句去掉，并去掉与 int_input 变量相关的所有语句，修改过后的 vul_prog.c 程序如下：
...
在 /home/shiyanlou 目录新建一个程序 write_string.c 。以下程序将一个格式化字符串写入了一个叫 mystring 的文件，前 4 个字节由任意你想放 入格式化字符串的数字构成，接下来的字节由键盘输入。
...
先运行 vul_prog 程序，输入 4 个 `%016llx` 。再运行 write_string 程序，输入 7 个 `%016llx` 和 1 个 `%n` ，此操作会生成一个 mystring 文件。然后输入如下命令：

    ./vul_prog < mystring

![此处输入图片的描述](https://doc.shiyanlou.com/document-uid8797labid734timestamp1526634870795.png/wm)
