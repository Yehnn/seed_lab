...
> Note :  如果你打算将明文存储在文件中读取，一些编辑器可能会擅自加奇怪的东西进去，比如 windows 的记事本，使用实验室环境的编辑器不会有这样的顾虑。

1、首先下载我们的词典到 /home/shiyanlou 目录：

```bash
$ cd /home/shiyanlou
$ wget http://www.cis.syr.edu/~wedu/seed/Labs_12.04/Crypto/Crypto_Encryption/files/words.txt
```

2、然后在 /home/shiyanlou 目录新建 fileKey.c 文件：
...
![此处输入图片的描述](https://doc.shiyanlou.com/document-uid8797labid788timestamp1527046187365.png)
...
- name: check word.txt
  ...
  error: 没有下载 words.txt 文件
- name: check fileKey.c
  script: |
    #!/bin/bash
    ls /home/shiyanlou/fileKey.c
  error: 没有 fileKey.c 文件
- name: check enc
  script: |
    #!/bin/bash
    ls /home/shiyanlou/enc
  error: 没有编译 fileKey.c
```
