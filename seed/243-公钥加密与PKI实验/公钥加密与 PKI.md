...
![3.1-1](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527139390893.png)
...
![3.1-2](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527139759720.png)
...
![3.1-3](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527140078774.png/wm)
...
![3.2-1](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527141768489.png)
...
![3.2-2](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527145694766.png)
...
![3.2-3](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527145768049.png)
...
![3.3-1](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527145952036.png/wm)

启动后不要关闭这个窗口。默认情况下，服务器会监听 4433 端口。所以你现在可以通过 https://PKILabServer.com:4433/ 访问服务器了。你很可能从浏览器收到一条错误消息: 

![3.3-2](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527146192007.png)
...
2. 我们可以手动加载我们 CA 的证书通过以下步骤：
  点击右边的三横线按钮-》首选项-》高级-》证书-》查看证书（也可以直接在地址栏输入 `about:preferances#advanced` 进入高级界面）
  你将看到一列证书列表，在这里我们导入我们的证书。 导入 ca.crt 并且选择 “Trust this CA to identify web sites” 。

![3.3-3](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527147058404.png/wm)

现在，浏览 https://PKILabServer.com:4433 请描述并解释你的观察。如果仍旧出现错误页面，请使用 opera 浏览器。

![3.3-4](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527147391468.png)
...
###3.4 使用 PKI 与 PKILabServer.com 建立安全 TCP 链接

在本实验中，我们将实现 TCP 客户端，服务器端及其安全连接。也就是说，客户端与服务器端使用一个只有他们自己知道的会话密钥对通信内容进行加密。另外，客户端需要确保连接的是目标服务器（`PKILabServer.com`）也就是说客户端需要给服务器端授权。服务器授权需要公钥证书，OpenSSL 已经实现了 SSL 协议来做服务器授权。OpenSSL 的 SSL 函数直接在客户端与服务器端之间建立连接，证书的认证由 SSL 函数自动完成。
...
![3.4-1](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527149110972.png/wm)

修改完成后执行 `make` 命令，会报下图的 warning，可以忽略。

![3.4-2](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527149093893.png/wm)
...
$ ./cli
# 打开另一个终端运行命令，输入密码 client
```

![3.4-3](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527149310937.png/wm)

![3.4-4](https://doc.shiyanlou.com/document-uid8797labid790timestamp1527149310236.png/wm)
...
本节实验中，我们将使用 OpenSSL 生成数字签名。请准备一个任意大小的 `example.txt` 文件。并准备 RSA 密钥对。
